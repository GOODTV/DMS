...
public partial class DonorMgr_DonorInfo_Print_Email : BasePage
{
    ...
    //---------------------------------------------------------------------------
    private void Print_Email(string strSql)
    {
...
//                                            Email as 電子信箱 ");
        strSql = strSql.Replace(strSql_temp[0],
                                 @" SELECT Distinct left(D.Emails,len(D.Emails)-1) as Email from
                                            (Select (SELECT cast(Email AS NVARCHAR ) + ',' 
                                            ");
        //20140425 修改 by Ian_Kao
        //strSql += " and Cellular_Phone != '' and Cellular_Phone is not null";
        strSql += " and Email <> '' and IsMember<>'Y' ";
        strSql += " order by Donor_Id desc ";
        //20140509 修改 by Ian_Kao 格式錯誤
        strSql += " FOR XML PATH('')) as Emails from DONOR D ) D ";
        ...
        //GetTableFooter();
        Util.OutputTxt(GridList.Text, "1", "email");
    ...
    private string GetTable1(DataRow dr, string strSql)
    {
        ...
        DataTable dtRet = CaseUtil.DonorInfo_Excel__Email(dt);

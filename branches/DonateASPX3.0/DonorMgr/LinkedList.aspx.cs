 ...
 */
public partial class DonorMgr_Linked_List : BasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        Session["ProgID"] = "LinkedList";
        ...
        if (!IsPostBack)
        {
            ...
            LinkList_DataBind();
            LinkItem_DataBind();
            tbxNewGiftItems_Seq.Text = CaseUtil.GetNewMaxLinked2_Seq(HFD_Linked_Id.Value);
        }
    }
    //---------------------------------------------------------------------------
    public void AuthrityControl()
    {
        ...
        Authrity.CheckButtonRight("_AddNew", btnNewGiftFolder);
        Authrity.CheckButtonRight("_AddNew", btnNewGiftItems);
        Authrity.CheckButtonRight("_Delete", btnDelGiftFolder);
    }
    //---------------------------------------------------------------------------
    protected void btnDelGiftFolder_Click(object sender, EventArgs e)
    {
        bool flag = false;
        try
        {
    ...
    //---------------------------------------------------------------------------
    protected void btnNewGiftItems_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            string strSql_check = "select * from Linked2 where Linked2_Name like '%" + tbxNewGiftItems.Text.Trim() + "%'";

            dt = NpoDB.GetDataTableS(strSql_check, dict_check);
            if (dt.Rows.Count != 0)
            {
                this.Page.RegisterStartupScript("s", "<script>alert('此「次分類名稱」已重複！！');</script>");
                flag = false;
                return;
            }
            try
            {
                ...
                dict.Add("Linked_Id", HFD_Linked_Id.Value);
                dict.Add("Linked2_Name", tbxNewGiftItems.Text.Trim());
                dict.Add("Linked2_Seq", tbxNewGiftItems_Seq.Text.Trim());
                NpoDB.ExecuteSQLS(strSql, dict);
    ...
    public void Inital()
    {
        ...
        {
            HFD_Linked_Id.Value = CaseUtil.GetOldMin_Gift_Linked_Id();
        }
        string strSql = @"select Linked_Id,Linked_Name as 公關贈品品項 from Linked where Linked_Type='Gift' and Linked_Id ='" + HFD_Linked_Id.Value + "'";
        Dictionary<string, object> dict = new Dictionary<string, object>();
        DataTable dt;
        dt = NpoDB.GetDataTableS(strSql, dict);
        DataRow dr = dt.Rows[0];
        tbxGiftFolder.Text = dr["公關贈品品項"].ToString();
    }
    //---------------------------------------------------------------------------
    public void LinkList_DataBind()
    {
        ...
        strSql = @"select Linked_Id,Linked_Name as 公關贈品品項,Linked_Seq as 排序 from Linked where Linked_Type='gift' order by 排序";
        dt = NpoDB.GetDataTableS(strSql, dict);
    ...
    private string CheckFieldMustFillBasic()
    {
        ...
        if (tbxNewGiftItems.Text.Trim() == "")
        {
            strRet += "次分類名稱 ";
        }
        if (tbxNewGiftItems_Seq.Text.Trim() == "")
        {
            strRet += "排序 ";
        }
        if (tbxNewGiftItems.Text.Trim() == "" || tbxNewGiftItems_Seq.Text.Trim() == "")
        {
            strRet += "欄位不可為空白！";
        }
        return strRet;

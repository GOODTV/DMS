    ...
    <table width="100%" border="0" cellpadding="1" cellspacing="1">
      <tr>
        <td class="td02-c" width="500">
        	&nbsp;
        	會員姓名：
        	<input type="text" name="Donor_Name" size="10" class="font9" value="<%=request("Donor_Name")%>" onKeypress='if(event.keyCode==13){JavaScript:Query_OnClick();}'>
          &nbsp;
          會員編號：
		      <input type="text" name="Member_No" size="10" class="font9" value="<%=request("Member_No")%>" onKeyUp="javascript:UCaseMemberNo();" onKeypress='if(event.keyCode==13){JavaScript:Query_OnClick();}'>
		      &nbsp;
		      身份別：
          <%
...
<%
  If request("Donor_Name")<>"" Or request("Member_No")<>"" Or request("Donor_Type")<>"" Then
    SQL1="Select Distinct Donor_Id,編號=(Case When Member_No<>'' Then Member_No Else CONVERT(nvarchar(20),Donor_Id) End),會員姓名=(Case When NickName<>'' Then Donor_Name+'('+NickName+')' Else Donor_Name End),類別=Category,身份別=Donor_Type,聯絡電話=(Case When Cellular_Phone<>'' Then Cellular_Phone Else Case When Tel_Office<>'' Then Tel_Office Else Tel_Home End End),通訊地址=(Case When DONOR.City='' Then Address Else Case When A.mValue<>B.mValue Then A.mValue+DONOR.ZipCode+B.mValue+Address Else A.mValue+DONOR.ZipCode+Address End End) " & _

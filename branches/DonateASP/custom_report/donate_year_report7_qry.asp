                        ...
                        <tr>
                          <td>
							...
							SQL1="SELECT D.Donor_Id AS '編號'  " & _
									...
									"FROM dbo.DONOR M " & _
									"INNER JOIN (SELECT Donor_Id ,SUM(Donate_Amt) AS Sum_Amt ,COUNT(Donor_Id) AS Times ,MAX(Donate_Amt) AS Max_Amt " & _
									"FROM DONATE "
                              '搜尋條件
                              'Donate_Where=""
							  ...
							  If Request("Donate_Purpose")<>"" Then
                                ...
                                Else
                                  ...
                                  Next
                                End If
                                If donatepurpose<>"" Then SQL1=SQL1&"And Donate_Purpose In "&donatepurpose&" GROUP BY Donor_Id) D "
                              Else
							  ...
							  If Request("First_DonateYear")<>"" Then 
								SQL1=SQL1&"WHERE DATEPART(YEAR,M.Begin_DonateDate) = '"&Request("First_DonateYear")&"' "
							  Else
								SQL1=SQL1&"WHERE DATEPART(YEAR,M.Begin_DonateDate) <> '' "
							  End IF

							  If Request("Last_Donate_Date")<>"" Then 
								SQL1=SQL1&"AND M.Last_DonateDate >= '"&Request("Last_Donate_Date")&"' "
							  Else
								SQL1=SQL1
							  End IF
                            ...
                            Session("First_DonateYear")=Request("First_DonateYear")
							Session("Last_Donate_Date")=Request("Last_Donate_Date")
                            ...
                            If request("action")="report" Then
					                    Response.Redirect "../include/movebar.asp?URL=../custom_report/donate_year_report7_rpt.asp"
					                  ElseIf request("action")="export" then
					                    Response.Redirect "donate_year_report7_rpt.asp?action=export"
					                  End If 

                          ...
                          <td>
                            ...
                            If request("SQL1")="" Then
								SQL1="SELECT D.Donor_Id AS '編號'  " & _
									...
									",D.Donate_Purpose AS '奉獻用途',D.Amt AS '累計奉獻金額' ,D.Times AS '奉獻次數' " & _
									...
									",Birthday AS '生日' ,Begin_DonateDate AS '首捐日期' ,Last_DonateDate AS '末捐日期' " & _
									"FROM dbo.DONOR M " & _
									"RIGHT JOIN (SELECT Donor_Id ,Donate_Purpose ,Sum(Donate_Amt) AS Amt ,Count(Donor_Id) AS Times " & _
												"FROM DONATE "
								  ...
								  If Request("Donate_Total_Amt")<>"" Then 
									SQL1=SQL1&"GROUP BY Donor_Id, Donate_Purpose HAVING SUM(Donate_Amt) >= '"&Request("Donate_Total_Amt")&"' ) D "
								  Else
									SQL1=SQL1&"GROUP BY Donor_Id, Donate_Purpose ) D "
								  End IF
                            ...
                            If request("action")="report" Then
					                    Response.Redirect "../include/movebar.asp?URL=../custom_report/donate_season_report3_rpt.asp"
					                  ElseIf request("action")="export" then
					                    Response.Redirect "donate_season_report3_rpt.asp?action=export"
					                  End If 

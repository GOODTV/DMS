<%
If Request("action")="help" Then Response.Redirect "address_list_help.htm"
...
If Request("Donor_Name")<>"" Then Donor_Where=Donor_Where&"And Donor_Name Like '%"&request("Donor_Name")&"%' "
If Request("Donor_Id_Begin")<>"" Then Donor_Where=Donor_Where&"And Donor.Donor_Id>='"&Request("Donor_Id_Begin")&"' "
If Request("Donor_Id_End")<>"" Then Donor_Where=Donor_Where&"And Donor.Donor_Id<='"&Request("Donor_Id_End")&"' "
If Request("Birthday_Month")<>"" Then Donor_Where=Donor_Where&"And Month(Birthday)='"&Request("Birthday_Month")&"' "
If Request("Donate_Over")<>"" Then Donor_Where=Donor_Where&"And Last_DonateDateD<'"&DateAdd("M",Cint(Request("Donate_Over"))*-1,Date())&"' "   
'20130730 Modify by GoodTV Tanya:修改名條用途項目
If Request("Print_Type")="2" Then Donor_Where=Donor_Where&"And IsSendNews='Y' "  '紙本月刊
If Request("Print_Type")="3" Then Donor_Where=Donor_Where&"And DATEPART(Month,Birthday) = '"&Request("BirthdayMonth")&"' "  '生日卡
If Request("Print_Type")="4" Then Donor_Where=Donor_Where&"And IsDVD='Y' "       'DVD	                          
'If Request("Print_Type")="2" Then Donor_Where=Donor_Where&"And IsSendNews='Y' "  '郵寄會訊
'If Request("Print_Type")="3" Then Donor_Where=Donor_Where&"And IsBirthday='Y' "  '郵寄年報
'If Request("Print_Type")="4" Then Donor_Where=Donor_Where&"And IsBirthday='Y' "  '郵寄生日卡
'If Request("Print_Type")="5" Then Donor_Where=Donor_Where&"And IsXmas='Y' "      '郵寄賀卡
...
If Request("Print_Type")="1" Then                    
  SQL1="Select Distinct Donor.Dept_Id,Donor.Donor_Id,Donor_Name,Title=(Case When Title='' Then '君' Else Title End),ZipCode,Address=(Case When DONOR.City='' Then Address Else Case When B.mValue<>C.mValue Then B.mValue+C.mValue+Address Else B.mValue+Address End End),Invoice_ZipCode,Invoice_Address=(Case When DONOR.Invoice_City='' Then Invoice_Address Else Case When D.mValue<>E.mValue Then D.mValue+E.mValue+Invoice_Address Else D.mValue+Invoice_Address End End),Invoice_Pre,Invoice_No " & _
       ...
       "Where Invoice_No<>'' And A.Invoice_Type Not In ('"&Request("InvoiceTypeN")&"','"&Request("InvoiceTypeY")&"') " & _
       " And Donor.Donor_Name <> '主知名' And Donor.Donor_Type Not Like '%主知名%' And IsNull(Donor.Invoice_Title,'') <> '主知名' " & _
...
Else
  ...
  SQL1="Select Distinct Donor.Dept_Id,Donor.Donor_Id,Donor_Name,Title=(Case When Title='' Then '君' Else Title End),ZipCode,Address=(Case When DONOR.City='' Then Address Else Case When B.mValue<>C.mValue Then B.mValue+C.mValue+Address Else B.mValue+Address End End),Invoice_ZipCode,Invoice_Address=(Case When DONOR.Invoice_City='' Then Invoice_Address Else Case When D.mValue<>E.mValue Then D.mValue+E.mValue+Invoice_Address Else D.mValue+Invoice_Address End End) " & _
       "From Donor "&Join_Type&" (Select Donor_Id,Donate_No=Count(*),Donate_Total=Sum(Donate_Amt) From Donate Where Issue_Type<>'D' "&replace(Donate_Where,"A.","Donate.")&" " &_
       "Group By Donor_Id) As A On Donor.Donor_Id=A.Donor_Id " & _
       "Left Join CODECITY As B On DONOR.City=B.mCode Left Join CODECITY As C On DONOR.Area=C.mCode " & _
       "Left Join CODECITY As D On DONOR.Invoice_City=D.mCode Left Join CODECITY As E On DONOR.Invoice_Area=E.mCode "
  If Request("Print_Desc")="1" Then
    SQL1=SQL1&"Where Invoice_Address<>'' " & _      
              " And Donor.Donor_Name <> '主知名' And Donor.Donor_Type Not Like '%主知名%' And IsNull(Donor.Invoice_Title,'') <> '主知名' " & _
    					" And IsNull(Donor.IsErrAddress,'') <> 'Y' And IsNull(Donor.IsContact,'') <> 'N' And IsNull(Donor.Sex,'') <> '歿' " & _                      
              Donor_Where & OrderBy_Where
  Else
    SQL1=SQL1&"Where Address<>'' " & _ 
    					" And Donor.Donor_Name <> '主知名' And Donor.Donor_Type Not Like '%主知名%' And IsNull(Donor.Invoice_Title,'') <> '主知名' " & _
    					" And IsNull(Donor.IsErrAddress,'') <> 'Y' And IsNull(Donor.IsContact,'') <> 'N' And IsNull(Donor.Sex,'') <> '歿' " & _
              Donor_Where & OrderBy_Where
  End If

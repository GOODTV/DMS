  ...
  If UploadName<>"" Then
    ...
    While Not RSX.EOF 
      If Trim(RSX(0))<>"" Then
        ...
        Excel_Type="import"
        Donor_Name=""
        If Trim(RSX(0))<>"" Then Donor_Name=Trim(RSX(0))
        IDNo=""
        If Trim(RSX(1))<>"" Then IDNo=Trim(RSX(1))
        If Len(IDNo)=7 Then IDNo="0"&IDNo
        Cellular_Phone=""
        If Trim(RSX(2))<>"" Then Cellular_Phone=Trim(RSX(2))
        Tel_Office=""
        If Trim(RSX(3))<>"" Then Tel_Office=Trim(RSX(3))
        Email=""
        If Trim(RSX(4))<>"" Then Email=Trim(RSX(4))
        ZipCode=Trim(RSX(5))
        City=""
        Area=""
        Address=""
        If Trim(RSX(6))<>"" Then Address=Trim(RSX(6))
        If Address<>"" Then
          ...
          Else
            ...
            RS1.Open SQL1,Conn,1,1
            If Not RS1.EOF Then 
              If ZipCode="" Then ZipCode=RS1("Area")
              City=RS1("City")
              Area=RS1("Area")
        ...
        Donate_Date=""
        If Trim(RSX(7))<>"" Then
          Donate_Date=Trim(RSX(7))
        Else
          Donate_Date=Dept_Date
        End If
        Donate_Payment=Trim(RSX(8))
        Donate_Purpose=Trim(RSX(9))
        Donate_Amt=Trim(RSX(10))
        InvoiceYN=Trim(RSX(11))
        Invoice_Title=Trim(RSX(12))
        If Invoice_Title="" Then Invoice_Title=Donor_Name
        ...
        If RS1.EOF Then
          ...
          RS1("IsXmas")="N"
          If InvoiceYN<>"" Then
            If InvoiceYN=InvoiceTypeY Or InvoiceYN=InvoiceTypeN Or InvoiceYN=InvoiceTypeT Then
              RS1("Invoice_type")=InvoiceYN
            Else
              RS1("Invoice_type")=InvoiceTypeY
            End If
          Else
            RS1("Invoice_type")=InvoiceTypeY
          End If
          If Address<>"" Then
            If InvoiceYN="Y" Or InvoiceYN="是" Or InvoiceYN="R" Then
              RS1("Invoice_type")=InvoiceTypeT
            ElseIf InvoiceYN="N" Or InvoiceYN="否" Then
        ...
        '捐款紀錄
        SQL1="Select * From DONATE Where Excel_Type='"&Excel_Type&"' And Donate_Date='"&Donate_Date&"' And CONVERT(numeric,Donate_Amt)='"&Donate_Amt&"' And Donate_Payment='"&Donate_Payment&"' And Donate_Purpose='"&Donate_Purpose&"' And Donor_Id='"&Donor_Id&"'"
        Set RS1=Server.CreateObject("ADODB.RecordSet")
        RS1.Open SQL1,Conn,1,3
        If RS1.EOF Then
          Invoice_Pre=""
          ...
          If InvoiceYN=InvoiceTypeN Or InvoiceYN=InvoiceTypeT Then
    ...
    Set ConnX=Nothing
    session("errnumber")=1
    session("msg")="捐款資料匯入成功 ！\n\n共計："&FormatNumber(Row,0)&"筆，金額："&FormatNumber(Total,0)&"元。"
  End If
End If
%>
<%Prog_Id="import_donate"%>
<!--#include file="../include/head.asp"-->
<body OnLoad='Window_OnLoad()'>
  <p><div align="center"><center>
    <form  name="form" action="import_donate.asp?action=upload" method="post" enctype="multipart/form-data">	
                  ...
                  　  </td>
                    ...
                    <tr>
                      ...
                      <td class="td02-c">
                        ...
                        <input type="button" value="匯入捐款資料" name="input" class="addbutton" style="cursor:hand" onClick="Input_OnClick()">
                      </td>
                    </tr>
                    <tr>
                      <td class="td02-c" align="right" width="150">※&nbsp;注意事項：</td>
                      <td class="td02-c" width="630">
                      	1.您選擇的『&nbsp;<font color="#FF0000">機構名稱</font>&nbsp;』會影響&nbsp;<font color="#FF0000">收據編號</font>&nbsp;取號規則，上傳前請您務必確認。<br />
                      	2.工作表名稱必須與檔案的工作表名稱一致否則將導致作業失敗。<br />
                      	3.檔案必須為&nbsp;Excel&nbsp;格式，如您的檔案來源自網站下載請務必&nbsp;<font color="#FF0000">另存新檔</font>&nbsp;，檔案大小請勿超過10M。<br />
                      	4.相關欄位說明請下載&nbsp;<a href='import/捐款匯入參考範例.xls'>參考範例</a>&nbsp;。
                      </td>
                    </tr>

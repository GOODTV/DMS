...
End If
If request("Valid_Month")<>"" Then SQL_Where=SQL_Where & "And Substring((Valid_Date),1,2) = '"&request("Valid_Month")&"' "
If request("Valid_Year")<>"" Then SQL_Where=SQL_Where & "And Substring((Valid_Date),3,2) = '"&Right(request("Valid_Year"),2)&"' "
...
SQL1="Select Distinct A.Dept_Id,A.Donor_Id,Donor_Name,Title=(Case When Title='' Then '君' Else Title End),ZipCode,Address=(Case When DONOR.City='' Then Address Else Case When B.mValue<>C.mValue Then B.mValue+C.mValue+Address Else B.mValue+Address End End),Invoice_ZipCode,Invoice_Address=(Case When DONOR.Invoice_City='' Then Invoice_Address Else Case When D.mValue<>E.mValue Then D.mValue+E.mValue+Invoice_Address Else D.mValue+Invoice_Address End End),Pledge_Id,Email,Valid_Date " & _
...
If Request("action")="report" Then
  ...
  Response.Redirect "../include/movebar.asp?URL=../donation/pledge_valid_rpt_style1.asp"

...
'20130917 Add by GoodTV-Tanya:增加只統計天使資料，與讀者切割(IsMember<>'Y')
...
If request("action")="export" Then
  ...
  Response.AddHeader "content-disposition", "attachment;filename=donor_stat.xls"
End If       
%>
...
<body class=tool>
  ...
  <table border="0" width="720" cellspacing="0" cellpadding="4">
    <tr>
			<td align="center" colspan='2'><span style="font-size: 14pt;font-family: 標楷體">捐款人<%=StatKind%>統計</span></td>
		</tr>
		<tr>
  ...
  <%
    ...
    For I=0 To UBound(Ary_CodeDesc)
      If Session("Stat_Kind")="Age" Then
        If I=0 Then
          ...
          SQL1="Select Donor_Total_P=Count(*) From DONOR Where Year(Birthday)>='"&Year(Date())-Cdbl(Ary_CodeDesc(I))&"'"
        ...
        Else
          ...
          SQL1="Select Donor_Total_P=Count(*) From DONOR Where Year(Birthday)>='"&Year(Date())-Cdbl(Ary_CodeDesc(I))&"' And Year(Birthday)<='"&Year(Date())-Cdbl(Ary_CodeDesc(I-1))-1&"'"
        End If
        
      ElseIf Session("Stat_Kind")="City" Or Session("Stat_Kind")="Invoice_City" Then
        CodeDesc=Ary_City(I)
        SQL1="Select Donor_Total_P=Count(*) From DONOR Where "&Session("Stat_Kind")&"='"&Ary_CodeDesc(I)&"'"
      Else
    ...
    '資料不詳
    If Session("Stat_Kind")="Age" Then
      SQL1="Select Donor_Total_P=Count(*) From DONOR Where (Birthday Is null)"
    Else
      SQL1="Select Donor_Total_P=Count(*) From DONOR Where ("&Session("Stat_Kind")&"='' Or "&Session("Stat_Kind")&" Is null)"
    End If

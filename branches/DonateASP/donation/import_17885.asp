  ...
  If UploadName<>"" Then
    ...
    While Not RSX.EOF
      If Trim(RSX(0))<>"" Then
        ...
        Excel_No=Trim(RSX(0))
        If Trim(RSX(11))<>"" Then
          Excel_No=Replace(Trim(RSX(11)),"?","")
          Remark=Remark&"智邦收據號碼："&Excel_No&vbcrlf
        End If
        Donate_Date=""
        If Dept_Date<>"" Then
        ...
        Else
          If Trim(RSX(1))<>"" Then
            Donate_Date=Replace(Trim(RSX(1)),"?","")
            Donate_Date=Left(Donate_Date,4)&"/"&Mid(Donate_Date,5,2)&"/"&Mid(Donate_Date,7,2)
          End If
        End If
        Donor_Name=""
        If Trim(RSX(2))<>"" Then Donor_Name=Replace(Trim(RSX(2)),"?","")
        Email=""
        If Trim(RSX(3))<>"" Then Email=Replace(Trim(RSX(3)),"?","")
        Donate_Amt="0"
        If Trim(RSX(4))<>"" Then Donate_Amt=Replace(Trim(RSX(4)),"?","")
        Tel_Office=""
        If Trim(RSX(5))<>"" Then Tel_Office=Replace(Trim(RSX(5)),"?","")
        Cellular_Phone=""
        If Trim(RSX(6))<>"" Then Cellular_Phone=Replace(Trim(RSX(6)),"?","")
        If Trim(RSX(7))<>"" Then Remark=Remark&"智邦款類:"&Replace(Trim(RSX(7)),"?","")&vbcrlf
        Invoice_Title=Donor_Name
        If Trim(RSX(8))<>"" Then Invoice_Title=Replace(Trim(RSX(8)),"?","")
        Invoice_IDNo=""
        If Trim(RSX(9))<>"" Then Invoice_IDNo=UCase(Replace(Trim(RSX(9)),"?",""))
        IDNo=""
        If Cstr(Donor_Name)=Cstr(Invoice_Title) Then IDNo=Invoice_IDNo
        If Trim(RSX(12))<>"" Then Remark=Remark&"智邦收據日期:"&Replace(Trim(RSX(12)),"?","")&vbcrlf
        If Trim(RSX(13))<>"" Then Remark=Remark&Replace(Trim(RSX(13)),"?","")&vbcrlf
        ZipCode=""
        City=""
        Area=""
        Address=""
        If Trim(RSX(10))<>"" Then Address=Replace(Trim(RSX(10)),"?","")
        If Address<>"" Then
          For I=1 To Len(Address)
            If IsNumeric(Mid(Address,I,1)) Then
              ZipCode=ZipCode&Mid(Address,I,1)
            Else
              If ZipCode<>"" Then
                Address=Trim(Replace(Address,ZipCode,""))
                SQL1="Select City_Name=B.mValue,Area_Name=A.mValue,City=B.mCode,Area=A.mCode From CODECITY A Join CODECITY B On Substring(A.codeMetaID,7,1)=B.mCode Where A.mCode='"&Left(ZipCode,3)&"'"
                Set RS1 = Server.CreateObject("ADODB.RecordSet")
                RS1.Open SQL1,Conn,1,1
                If Not RS1.EOF Then
                  City=RS1("City")
                  Area=RS1("Area")
                  If RS1("City_Name")<>"" Then Address=Trim(Replace(Address,RS1("City_Name"),""))
                  If RS1("Area_Name")<>"" Then Address=Trim(Replace(Address,RS1("Area_Name"),""))
                End If
                RS1.Close
                Set RS1=Nothing 
              End If
        ...
        SQL1="Select * From DONATE Where Excel_Type='"&Excel_Type&"' And Excel_No='"&Excel_No&"' And Donor_Id='"&Donor_Id&"' And Donate_Date='"&Donate_Date&"' And CONVERT(numeric,Donate_Amt)='"&Donate_Amt&"'"
        Set RS1=Server.CreateObject("ADODB.RecordSet")
        RS1.Open SQL1,Conn,1,3
        If RS1.EOF Then
          Invoice_Pre=""
    ...
    Set ConnX=Nothing
    session("errnumber")=1
    session("msg")="智邦捐款資料匯入成功 ！\n\n共計："&FormatNumber(Row,0)&"筆，金額："&FormatNumber(Total,0)&"元。"
  End If
End If
%>
<%Prog_Id="import_17885"%>
<!--#include file="../include/head.asp"-->
<body OnLoad='Window_OnLoad()'>
<body>
  <p><div align="center"><center>
    <form  name="form" action="import_17885.asp?action=upload" method="post" enctype="multipart/form-data">	
                  ...
                  　  </td>
                    ...
                    <tr>
                      ...
                      <td class="td02-c">
                        ...
                        <input type="button" value="匯入『&nbsp;智邦捐款&nbsp;』資料" name="input" class="addbutton" style="cursor:hand" onClick="Input_OnClick()">
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td class="td02-c" width="630">
                      	...
                      	4.相關欄位說明請下載&nbsp;<a href='import/智邦公益參考範例.xls'>參考範例</a>&nbsp;。
                      </td>
                    </tr>

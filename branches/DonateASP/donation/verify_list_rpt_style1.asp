...
Function DonateList (SQL,ReportName)
  ...
  While Not RS1.EOF
	  Response.Write "<tr>"
	  For I = 1 To FieldsCount
      If I=3 And Session("Donate_Payment")="1" Then
        If RS1(I)<>"" Then
...
<%
  If Session("Donate_Payment")="1" Then
    SQL="Select Donate_Id,捐款日期=Donate_Date,捐款人=(Case When IsAnonymous='Y' Then Case When NickName<>'' Then NickName Else Case When Donate.Invoice_Title<>'' Then Donate.Invoice_Title Else Donor_Name End End Else Case When Donate.Invoice_Title<>'' Then Donate.Invoice_Title Else Donor_Name End End),捐款金額=Donate_Amt "
  Else
    SQL="Select Donate_Id,捐款日期=Donate_Date,捐款人=(Case When IsAnonymous='Y' Then Case When NickName<>'' Then NickName Else Case When Donate.Invoice_Title<>'' Then Donate.Invoice_Title Else Donor_Name End End Else Case When Donate.Invoice_Title<>'' Then Donate.Invoice_Title Else Donor_Name End End),捐物內容=CONVERT(nvarchar(4000),Donate_Desc) "
  End If

...
If request("action")="upload" Then
  ...
  If UploadName<>"" Then
    ...
    While Not RSX.EOF
      If Trim(RSX(0))<>"" Then
        Remark=""
        Donate_Payment="Yahoo公益"
        Excel_Type="yahoo"
        Excel_No=""
        If Trim(RSX(0))<>"" Then
          Excel_No=Trim(RSX(0))
          Remark=Remark&"訂單號碼："&Excel_No&vbcrlf
        End If
        Donate_Amt="0"
        If Trim(RSX(1))<>"" Then Donate_Amt=Trim(RSX(1))
        Donor_Name=""
        If Trim(RSX(2))<>"" Then Donor_Name=Trim(RSX(2))
        Tel_Office=""
        If Trim(RSX(3))<>"" Then Tel_Office=Trim(RSX(3))
        Cellular_Phone=""
        If Tel_Office<>"" Then
          If Len(Tel_Office)=9 And Left(Tel_Office,1)="9" Then
            Cellular_Phone="0"&Tel_Office
            Tel_Office=""
          End If
        End If
        IsAbroad="N"
        If Trim(RSX(4))<>"國內" Then IsAbroad="Y"
        ZipCode=""
        City=""
        Area=""
        Address=Trim(RSX(8))
        If IsAbroad="Y" Then
          Address=Trim(RSX(5))&Trim(RSX(6))&Trim(RSX(7))&Trim(RSX(8))
        Else
          If Trim(RSX(5))<>"" Then
            ZipCode=Trim(RSX(5))
            Area=Left(Trim(RSX(5)),3)
            SQL1="Select City=Right(codeMetaID,1) From CODECITY Where mCode='"&Area&"'"
            Set RS1 = Server.CreateObject("ADODB.RecordSet")
            RS1.Open SQL1,Conn,1,1
            If Not RS1.EOF Then City=RS1("City")
            RS1.Close
            Set RS1=Nothing    
          Else
            Address=Trim(RSX(5))&Trim(RSX(6))&Trim(RSX(7))&Trim(RSX(8))
        ...
        End If
        IDNo=""
        Invoice_Title=Trim(RSX(9))
        If Invoice_Title="" Then Invoice_Title=Donor_Name
        Invoice_IDNo=Trim(RSX(10))
        If Cstr(Invoice_Title)=Cstr(Donor_Name) Then IDNo=Invoice_IDNo
        Email=""
        If Trim(RSX(11))<>"" Then Email=Trim(RSX(11))
        If Trim(RSX(12))<>"" Then Remark=Remark&"持卡人姓名："&Trim(RSX(12))&vbcrlf
        If Trim(RSX(13))<>"" Then Remark=Remark&"卡號："&Trim(RSX(13))&vbcrlf
        If Trim(RSX(14))<>"" Then Remark=Remark&"授權碼："&Trim(RSX(14))&vbcrlf
        Donate_Date=""
        If Dept_Date<>"" Then
         Donate_Date=Dept_Date
        Else
          If Trim(RSX(15))<>"" Then Donate_Date=Left(Trim(RSX(15)),4)&"/"&Mid(Trim(RSX(15)),6,2)&"/"&Mid(Trim(RSX(15)),9,2)
        End If
        
        Act_Id=""
        SQL1="Select * From ACT Where Act_Name Like '%"&Trim(RSX(16))&"%' Or Act_ShortName Like '%"&Trim(RSX(16))&"%'"
        Set RS1 = Server.CreateObject("ADODB.RecordSet")
        RS1.Open SQL1,Conn,1,1
        If Not RS1.EOF Then Act_Id=RS1("Act_Id")
        RS1.Close
        Set RS1=Nothing
        Donate_Purpose=Trim(RSX(17)) 
        
        IsAbroad_Invoice=IsAbroad
        ...
        If RS1.EOF Then
          ...
          End If
          RS1("IsAbroad_Invoice")=IsAbroad_Invoice
        ...
        If RS1.EOF Then
          Invoice_Pre=""
          Invoice_No=""
          InvoiceNo=Get_Invoice_No2("1",Dept_Id,Donate_Date,InvoiceTypeT,Act_Id)
          If InvoiceNo<>"" Then
    ...
    Set ConnX=Nothing
    session("errnumber")=1
    session("msg")="Yahoo捐款資料匯入成功 ！\n\n共計："&FormatNumber(Row,0)&"筆，金額："&FormatNumber(Total,0)&"元。"
  End If
End If
%>
<%Prog_Id="import_yahoo"%>
<!--#include file="../include/head.asp"-->
<body OnLoad='Window_OnLoad()'>
  <p><div align="center"><center>
    <form  name="form" action="import_yahoo.asp?action=upload" method="post" enctype="multipart/form-data">	
                  ...
                  　  </td>
                    ...
                    <tr>
                      ...
                      <td class="td02-c">
                        ...
                        <input type="button" value="匯入『&nbsp;Yahoo捐款&nbsp;』資料" name="input" class="addbutton" style="cursor:hand" onClick="Input_OnClick()">
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td class="td02-c" width="630">
                      	...
                      	4.相關欄位說明請下載&nbsp;<a href='import/Yahoo公益參考範例.xls'>參考範例</a>&nbsp;。
                      </td>
                    </tr>
...
function Input_OnClick(){
  ...
  if(confirm('您是否確定要匯入『Yahoo捐款』資料？\n\n※請注意匯入過程中請勿關閉視窗')){

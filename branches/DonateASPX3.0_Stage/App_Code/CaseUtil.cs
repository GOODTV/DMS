...
public class CaseUtil
{
    ...
    public static string MakeMenu(string Donor_Id, int SelectedTab)
    {
        ...
        Btn.TabNo = 1;
        Btn.Text = "捐款人資料";
        Btn.Style = "style='width:19%'";  //需要不同寬度時,可以在此設定新值
        ...
        Btn.Style = "style='width:19%'";  //需要不同寬度時,可以在此設定新值
        Btn.ImgSrc = "../images/toolbar_modify.gif";
        Btn.OnClick = "../DonateMgr/QuestionnaireDataList.aspx?Donor_Id=" + Donor_Id;
        ButtonList.Add(Btn);
    ...
    public static DataTable DonorQry_Print(DataTable dtSrc)
    {
        ...
        for (int i = 0; i < row; i++)
        {
            ...
            dr["性別"] = dtSrc.Rows[i][3].ToString();
            dr["身份別"] = dtSrc.Rows[i][4].ToString();
            dr["電話日"] = dtSrc.Rows[i][5].ToString();
            dr["手機"] = dtSrc.Rows[i][6].ToString();
            dr["電子信箱"] = dtSrc.Rows[i][7].ToString();
            dr["通訊地址"] = dtSrc.Rows[i][8].ToString();
            dr["最近捐款日期"] = dtSrc.Rows[i][9].ToString();
            dr["累積金額"] = dtSrc.Rows[i][10].ToString();
            dr["累積次數"] = dtSrc.Rows[i][11].ToString();
            dr["月刊本數"] = dtSrc.Rows[i][12].ToString();
            dtRet.Rows.Add(dr);
        }
    ...
    public static DataTable DonorQry_Print_Excel(DataTable dtSrc)
    {
        ...
        dtRet.Columns.Add("通訊地址");
        //dtRet.Columns.Add("是否錯址");
        //dtRet.Columns.Add("是否不主動聯絡");
        //dtRet.Columns.Add("是否刊登徵信錄");
        //dtRet.Columns.Add("海外地址");
        //dtRet.Columns.Add("紙本月刊");
        //dtRet.Columns.Add("DVD");
        //dtRet.Columns.Add("電子文宣");
        //dtRet.Columns.Add("大額謝卡");
        //dtRet.Columns.Add("郵簡");
        ...
        for (int i = 0; i < row; i++)
        {
            ...
            // 2014/4/8 顯示捐款人編號
            dr["捐款人編號"] = dtSrc.Rows[i]["捐款人編號"].ToString();

            dr["捐款人"] = dtSrc.Rows[i]["捐款人"].ToString();
            dr["性別"] = dtSrc.Rows[i]["性別"].ToString();
            dr["稱謂"] = dtSrc.Rows[i]["稱謂"].ToString();
            dr["身份別"] = dtSrc.Rows[i]["身分別"].ToString();
            ...
            dr["手機"] = dtSrc.Rows[i]["手機"].ToString();
            dr["電話日"] = dtSrc.Rows[i]["電話日"].ToString();
            //dr["電話夜"] = dtSrc.Rows[i]["Tel_Home"].ToString();
            dr["電子信箱"] = dtSrc.Rows[i]["電子信箱"].ToString();
            //dr["聯絡人"] = dtSrc.Rows[i]["Contactor"].ToString();
            //dr["服務單位"] = dtSrc.Rows[i]["OrgName"].ToString();
            //dr["職稱"] = dtSrc.Rows[i]["JobTitle"].ToString();
            dr["通訊地址"] = dtSrc.Rows[i]["通訊地址"].ToString();
            //dr["是否錯址"] = dtSrc.Rows[i]["IsErrAddress"].ToString();
            //if (dtSrc.Rows[i]["IsContact"].ToString() == "N")
            //{
            //    dr["是否不主動聯絡"] = "是";
            //}
            //else
            ...
            //    dr["是否不主動聯絡"] = "否";
            //}
            //if (dtSrc.Rows[i]["IsAnonymous"].ToString() == "N")
            //{
            //    dr["是否刊登徵信錄"] = "是";
            //}
            //else
            ...
            //dr["大額謝卡"] = dtSrc.Rows[i]["IsBigAmtThank"].ToString();
            //dr["郵簡"] = dtSrc.Rows[i]["IsPost"].ToString();
            ...
            //    dr["首次捐款日期"] = "";
            //}
            if (dtSrc.Rows[i]["最近捐款日"].ToString().Trim() != "")
            {
                dr["最近捐款日期"] = DateTime.Parse(dtSrc.Rows[i]["最近捐款日"].ToString().Trim()).ToShortDateString().ToString();
            }
            else
            {
                dr["最近捐款日期"] = "";
            }
            dr["累積次數"] = dtSrc.Rows[i]["累積次數"].ToString();
            dr["累積金額"] = Util.ToThree(dtSrc.Rows[i]["累積金額"].ToString(), "0");
            dr["月刊本數"] = dtSrc.Rows[i]["月刊本數"].ToString();
            dtRet.Rows.Add(dr);
        }
    ...
    public static DataTable DonorReport_Condition(DataTable dtSrc, String Condition)
    {
        ...
        else if (Condition == "8")
        {
            ...
            string[] cla = { "基隆市", "台北市", "新北市", "桃園市", "新竹市", "新竹縣", "苗栗縣", "台中市", "彰化縣", "南投縣", "雲林縣", "嘉義市", "嘉義縣", "台南市", "高雄市", "屏東縣", "宜蘭縣", "花蓮縣", "台東縣", "澎湖縣", "金門縣", "連江縣", "通訊縣市不詳", "合計" };
    ...
    public static DataTable DonateNameList_Print(DataTable dtSrc)
    {
        ...
        for (int i = 0; i < row; i++)
        {
            ...
            dr["捐款金額"] = dtSrc.Rows[i][3].ToString();
            dr["捐款方式"] = dtSrc.Rows[i][4].ToString();
            dr["捐款用途"] = dtSrc.Rows[i][5].ToString();
            dr["收據開立"] = dtSrc.Rows[i][6].ToString();
            dr["收據編號"] = dtSrc.Rows[i][7].ToString();
            dr["捐款類別"] = dtSrc.Rows[i][8].ToString();
            dr["狀態"] = dtSrc.Rows[i][9].ToString();
            dtRet.Rows.Add(dr);
        }
    ...
    public static DataTable Donate_Week_Report2_Print(DataTable dtSrc)
    {
        ...
        for (int i = 0; i < row; i++)
        {
            ...
            {
                dr["編號"] = dtSrc.Rows[i][1].ToString();
                dr["姓名"] = dtSrc.Rows[i][4].ToString() + dtSrc.Rows[i][3].ToString();
                dr["首捐日期"] = "";
                dr["累計奉獻<br>金額"] = "";
                dr["奉獻<br>次數"] = "";
            ...
            else
            {
                ...
                dr["手機"] = dtSrc.Rows[i][8].ToString();
                dr["Email"] = dtSrc.Rows[i][9].ToString();
                dr["月刊寄送"] = dtSrc.Rows[i][10].ToString();
                dr["不主動聯絡"] = dtSrc.Rows[i][11].ToString();
            }
            dtRet.Rows.Add(dr);
        };
        return dtRet;
    ...
    public static DataTable Donate_Week_Report4_Print(DataTable dtSrc, DataTable dtSrc2, bool XLS)
    {
        ...
        for (int i = 0; i <= row; i++)
        {
            ...
            else
            {
                ...
                dr["電話"] = dtSrc.Rows[i][11].ToString();
                dr["手機"] = dtSrc.Rows[i][12].ToString();
                dr["Email"] = dtSrc.Rows[i][13].ToString();
            }
            dtRet.Rows.Add(dr);
        };
        return dtRet;
    ...
    public static DataTable Donate_Month_Report1_Print(DataTable dtSrc)
    {
        ...
        for (int i = 0; i < row; i++)
        {
            ...
            {
                dr["編號"] = dtSrc.Rows[i][1].ToString();
                dr["姓名"] = dtSrc.Rows[i][5].ToString() + dtSrc.Rows[i][2].ToString();
                dr["稱謂"] = "";
                dr["身分別"] = "";
                dr["累計奉獻<br>金額"] = "";
                dr["奉獻<br>次數"] = "";
    ...
    public static DataTable Donate_Month_Report2_Print(DataTable dtSrc)
    {
        ...
        for (int i = 0; i < row; i++)
        {
            ...
            else
            {
                ...
                dr["編號"] = dtSrc.Rows[i][8].ToString();
                dr["姓名"] = dtSrc.Rows[i][9].ToString();
                dr["稱謂"] = dtSrc.Rows[i][10].ToString();
                dr["電話"] = dtSrc.Rows[i][11].ToString();
                dr["手機"] = dtSrc.Rows[i][12].ToString();
                dr["郵遞<br>區號"] = dtSrc.Rows[i][13].ToString();
                dr["地址"] = dtSrc.Rows[i][14].ToString();
            }
            dtRet.Rows.Add(dr);
        };
        return dtRet;
    ...
    public static DataTable Donate_Month_Report3_Print(DataTable dtSrc)
    {
        ...
        for (int i = 0; i < row; i++)
        {
            ...
            if (i == row - 1)
            {
                dr["編號"] = "總筆數：" + dtSrc.Rows[i][1].ToString() + "筆";
                dr["姓名"] = "";
                dr["郵遞區號"] = "";
                dr["地址"] = "";
                dr["Email"] = "";
                dr["身分別"] = "月刊總數：";
                dr["月刊份數"] = dtSrc.Rows[i][6].ToString();
                dr["不主動聯絡"] = "";
            }
            else
            {
                ...
                dr["Email"] = dtSrc.Rows[i][4].ToString();
                dr["身分別"] = dtSrc.Rows[i][5].ToString();
                dr["月刊份數"] = dtSrc.Rows[i][6].ToString();
                dr["不主動聯絡"] = dtSrc.Rows[i][7].ToString();
            }
            dtRet.Rows.Add(dr);
        };
        return dtRet;
    ...
    public static DataTable Donate_Season_Report2_Print(DataTable dtSrc)
    {
        ...
        for (int i = 0; i < row; i++)
        {
            ...
            else
            {
                ...
                dr["郵遞<br>區號"] = dtSrc.Rows[i][6].ToString();
                dr["地址"] = dtSrc.Rows[i][7].ToString();
                dr["Email"] = dtSrc.Rows[i][8].ToString();
                dr["首捐日期"] = dtSrc.Rows[i][9].ToString();
                dr["末捐日期"] = dtSrc.Rows[i][10].ToString();
            }
            dtRet.Rows.Add(dr);
        };
        return dtRet;
    ...
    public static DataTable Donate_Season_Report4_Print(DataTable dtSrc)
    {
        ...
        for (int i = 0; i < row; i++)
        {
            ...
            else
            {
                ...
                dr["刊物<br>原則"] = dtSrc.Rows[i][13].ToString();
                dr["月刊<br>份數"] = dtSrc.Rows[i][14].ToString();
            }
            dtRet.Rows.Add(dr);
        };
        return dtRet;
    ...
    public static DataTable Donate_Year_Report1_Print(DataTable dtSrc)
    {
        ...
        for (int i = 0; i < row; i++)
        {
            ...
            else
            {
                ...
                dr["郵遞<br>區號"] = dtSrc.Rows[i][5].ToString();
                dr["地址"] = dtSrc.Rows[i][6].ToString();
                dr["Email"] = dtSrc.Rows[i][7].ToString();
    ...
    public static DataTable Donate_Year_Report2_Print(DataTable dtSrc)
    {
        ...
        for (int i = 0; i < row; i++)
        {
            ...
            else
            {
                ...
                dr["稱謂"] = dtSrc.Rows[i][2].ToString();
                dr["身分別"] = dtSrc.Rows[i][3].ToString();
                dr["身分證或統編"] = dtSrc.Rows[i][4].ToString();
                dr["累計奉獻<br>金額"] = dtSrc.Rows[i][5].ToString();
                dr["奉獻<br>次數"] = dtSrc.Rows[i][6].ToString();
                dr["月刊<br>份數"] = dtSrc.Rows[i][7].ToString();
                dr["地址"] = dtSrc.Rows[i][8].ToString();
                dr["首捐日期"] = dtSrc.Rows[i][9].ToString();
                dr["末捐日期"] = dtSrc.Rows[i][10].ToString();
            }
    ...
    //-------------------------------------------------------------------------------------------------------------
    //年度收據列印報表
    public static DataTable Donate_Year_Report10_Print(DataTable dtSrc)
    {
        DataTable dtRet = new DataTable();
        ...
        for (int i = 0; i < row; i++)
        {
            ...
            else*/
            {
                dr["捐款人編號"] = dtSrc.Rows[i][0].ToString();
                dr["捐款人"] = dtSrc.Rows[i][1].ToString();
                dr["收據抬頭"] = dtSrc.Rows[i][2].ToString();
                dr["稱謂"] = dtSrc.Rows[i][3].ToString();
                dr["性別"] = dtSrc.Rows[i][4].ToString();
                dr["身份證/統編"] = dtSrc.Rows[i][5].ToString();
                dr["收據開立"] = dtSrc.Rows[i][6].ToString();
                dr["累計奉獻<br>金額"] = dtSrc.Rows[i][7].ToString();
                dr["奉獻<br>次數"] = dtSrc.Rows[i][8].ToString();
                dr["通訊地址"] = dtSrc.Rows[i][9].ToString();
                dr["收據地址"] = dtSrc.Rows[i][10].ToString();
            }
            dtRet.Rows.Add(dr);
        };
        return dtRet;
    ...
    public static DataTable Donate_Other_Report3_Print(DataTable dtSrc)
    {
        ...
        for (int i = 0; i < row; i++)
        {
            ...
            dr["Email"] = dtSrc.Rows[i][3].ToString();
            dr["付款方式"] = dtSrc.Rows[i][4].ToString();
            dr["狀態"] = dtSrc.Rows[i][5].ToString();
            dr["捐款時間"] = dtSrc.Rows[i][6].ToString();
            dr["電話"] = dtSrc.Rows[i][7].ToString();
            dr["手機"] = dtSrc.Rows[i][8].ToString();
            dr["通訊地址"] = dtSrc.Rows[i][9].ToString();
            dr["收據地址"] = dtSrc.Rows[i][10].ToString();
    ...
    //---------------------------------------------------------------------------
    //捐款人/讀者綜合明細
    public static DataTable Donate_Other_Report4_Print(DataTable dtSrc)
    {
        DataTable dtRet = new DataTable();
        ...
        dtRet.Columns.Add("姓名");
        dtRet.Columns.Add("性別");
        dtRet.Columns.Add("年齡");
        dtRet.Columns.Add("身份別");
        dtRet.Columns.Add("地址");
        dtRet.Columns.Add("電話");
        dtRet.Columns.Add("手機");
        dtRet.Columns.Add("Email");
        dtRet.Columns.Add("奉獻方式");
        dtRet.Columns.Add("累計奉獻金額");
        dtRet.Columns.Add("奉獻次數");
        dtRet.Columns.Add("首捐日期");
        dtRet.Columns.Add("末捐日期");
        dtRet.Columns.Add("月刊寄否");
        dtRet.Columns.Add("月刊份數");
        dtRet.Columns.Add("不主動聯絡");
        dtRet.Columns.Add("是否錯址");
        int row = dtSrc.Rows.Count;
        ...
        for (int i = 0; i < row; i++)
        {
            ...
            dr["姓名"] = dtSrc.Rows[i][1].ToString();
            dr["性別"] = dtSrc.Rows[i][2].ToString();
            dr["年齡"] = dtSrc.Rows[i][3].ToString();
            dr["身份別"] = dtSrc.Rows[i][4].ToString();
            dr["地址"] = dtSrc.Rows[i][5].ToString();
            dr["電話"] = dtSrc.Rows[i][6].ToString();
            dr["手機"] = dtSrc.Rows[i][7].ToString();
            dr["Email"] = dtSrc.Rows[i][8].ToString();
            dr["奉獻方式"] = dtSrc.Rows[i][9].ToString();
            dr["累計奉獻金額"] = dtSrc.Rows[i][10].ToString();
            dr["奉獻次數"] = dtSrc.Rows[i][11].ToString();
            dr["首捐日期"] = dtSrc.Rows[i][12].ToString();
            dr["末捐日期"] = dtSrc.Rows[i][13].ToString();
            dr["月刊寄否"] = dtSrc.Rows[i][14].ToString();
            dr["月刊份數"] = dtSrc.Rows[i][15].ToString();
            dr["不主動聯絡"] = dtSrc.Rows[i][16].ToString();
            dr["是否錯址"] = dtSrc.Rows[i][17].ToString();
    ...
    //---------------------------------------------------------------------------
    //捐款人/讀者統計查詢報表
    public static DataTable Donate_Other_Report5_Print(DataTable dtSrc)
    {
        DataTable dtRet = new DataTable();

        dtRet.Columns.Add("項目");
        dtRet.Columns.Add("統計人數");
        dtRet.Columns.Add("百分比");
        int row = dtSrc.Rows.Count;
        ...
        for (int i = 0; i < row; i++)
        {
            ...
            dr["項目"] = dtSrc.Rows[i][0].ToString();
            dr["統計人數"] = dtSrc.Rows[i][1].ToString();
            dr["百分比"] = dtSrc.Rows[i][2].ToString();
    ...
    //---------------------------------------------------------------------------
    //捐款人統計分析查詢報表
    public static DataTable Donate_Other_Report6_Print(DataTable dtSrc, String Condition)
    {
        DataTable dtRet = new DataTable();
        if (Condition == "1")
        {
        ...
        {
            dtRet.Columns.Add("身份別統計");
            dtRet.Columns.Add("人數");
            dtRet.Columns.Add("百分比");


            string[] cla = { "個人", "公司行號", "教會", "機構", "醫院", "學校", "合計" };
        ...
        {
            dtRet.Columns.Add("通訊資料統計");
            dtRet.Columns.Add("人數");
            dtRet.Columns.Add("百分比");


            string[] cla = { "有手機", "有市內電話", "有Email", "合計" };
        ...
        {
            dtRet.Columns.Add("訂閱月刊統計");
            dtRet.Columns.Add("人數");
            dtRet.Columns.Add("百分比");


            string[] cla = { "有訂月刊", "未訂月刊", "合計" };
        ...
        {
            dtRet.Columns.Add("奉獻金額統計");
            dtRet.Columns.Add("人數");
            dtRet.Columns.Add("百分比");


            string[] cla = { "一億以上", "一億以下～1000萬以上", "1000萬以下～800萬以上", "800萬以下～700萬以上", "700萬以下～600萬以上", "600萬以下～500萬以上", "500萬以下～400萬以上", "400萬以下～300萬以上", "300萬以下～200萬以上", "200萬以下～100萬以上", "100萬以下～90萬以上", "90萬以下～80萬以上", "80萬以下～70萬以上", "70萬以下～60萬以上", "60萬以下～50萬以上", "50萬以下～40萬以上", "40萬以下～30萬以上", "30萬以下～20萬以上", "20萬以下～10萬以上", "10萬以下～9萬以上", "9萬以下～8萬以上", "8萬以下～7萬以上", "7萬以下～6萬以上", "6萬以下～5萬以上", "5萬以下～4萬以上", "4萬以下～3萬以上", "3萬以下～2萬以上", "2萬以下～1萬以上", "1萬以下", "合計" };
        ...
        {
            dtRet.Columns.Add("奉獻次數統計");
            dtRet.Columns.Add("人數");
            dtRet.Columns.Add("百分比");


            string[] cla = { "300次以上", "200次以上", "150次以上", "100次以上", "90次以上", "80次以上", "70次以上", "60次以上", "50次以上", "40次以上", "30次以上", "20次以上", "15次以上", "10次以上", "9次", "8次", "7次", "6次", "5次", "4次", "3次", "2次", "1次", "合計" };
    ...
    //---------------------------------------------------------------------------
    //奉獻動機及收視管道統計分析
    public static DataTable Donate_Other_Report7_Print(DataTable dtSrc)
    {
        DataTable dtRet = new DataTable();

        dtRet.Columns.Add("日期");
        dtRet.Columns.Add("支持媒體宣教大平台，可廣傳福音");
        dtRet.Columns.Add("個人靈命得造就");
        dtRet.Columns.Add("支持優質節目製作");
        dtRet.Columns.Add("支持GOOD TV家庭事工");
        dtRet.Columns.Add("感恩奉獻");
        dtRet.Columns.Add("其他");
        dtRet.Columns.Add("GOOD TV電視頻道");
        dtRet.Columns.Add("官網");
        dtRet.Columns.Add("Facebook");
        dtRet.Columns.Add("Youtube");
        dtRet.Columns.Add("好消息月刊");
        dtRet.Columns.Add("GOOD TV簡介刊物");
        dtRet.Columns.Add("教會牧者");
        dtRet.Columns.Add("親友");
        dtRet.Columns.Add("報章雜誌");
        int row = dtSrc.Rows.Count;
        ...
        for (int i = 0; i < row; i++)
        {
            ...
            dr["日期"] = dtSrc.Rows[i][0].ToString();
            dr["支持媒體宣教大平台，可廣傳福音"] = dtSrc.Rows[i][1].ToString();
            dr["個人靈命得造就"] = dtSrc.Rows[i][2].ToString();
            dr["支持優質節目製作"] = dtSrc.Rows[i][3].ToString();
            dr["支持GOOD TV家庭事工"] = dtSrc.Rows[i][4].ToString();
            dr["感恩奉獻"] = dtSrc.Rows[i][5].ToString();
            dr["其他"] = dtSrc.Rows[i][6].ToString();
            dr["GOOD TV電視頻道"] = dtSrc.Rows[i][7].ToString();
            dr["官網"] = dtSrc.Rows[i][8].ToString();
            dr["Facebook"] = dtSrc.Rows[i][9].ToString();
            dr["Youtube"] = dtSrc.Rows[i][10].ToString();
            dr["好消息月刊"] = dtSrc.Rows[i][11].ToString();
            dr["GOOD TV簡介刊物"] = dtSrc.Rows[i][12].ToString();
            dr["教會牧者"] = dtSrc.Rows[i][13].ToString();
            dr["親友"] = dtSrc.Rows[i][14].ToString();
            dr["報章雜誌"] = dtSrc.Rows[i][15].ToString();
    ...
    //---------------------------------------------------------------------------
    //個別奉獻動機及收視管道統計分析
    public static DataTable Donate_Other_Report8_Print(DataTable dtSrc)
    {
        DataTable dtRet = new DataTable();
        ...
        dtRet.Columns.Add("親友");
        dtRet.Columns.Add("報章雜誌");
        dtRet.Columns.Add("給GOODTV的話");
        dtRet.Columns.Add("問卷來源");
        ...
        for (int i = 0; i < row; i++)
        {
            ...
            dr["日期"] = dtSrc.Rows[i][2].ToString();
            dr["支持媒體宣教大平台，可廣傳福音"] = dtSrc.Rows[i][3].ToString();
            dr["個人靈命得造就"] = dtSrc.Rows[i][4].ToString();
            dr["支持優質節目製作"] = dtSrc.Rows[i][5].ToString();
            dr["支持GOOD TV家庭事工"] = dtSrc.Rows[i][6].ToString();
            dr["感恩奉獻"] = dtSrc.Rows[i][7].ToString();
            dr["GOOD TV電視頻道"] = dtSrc.Rows[i][8].ToString();
            dr["官網"] = dtSrc.Rows[i][9].ToString();
            dr["Facebook"] = dtSrc.Rows[i][10].ToString();
            dr["Youtube"] = dtSrc.Rows[i][11].ToString();
            dr["好消息月刊"] = dtSrc.Rows[i][12].ToString();
            dr["GOOD TV簡介刊物"] = dtSrc.Rows[i][13].ToString();
            dr["教會牧者"] = dtSrc.Rows[i][14].ToString();
            dr["親友"] = dtSrc.Rows[i][15].ToString();
            dr["報章雜誌"] = dtSrc.Rows[i][16].ToString();
            dr["給GOODTV的話"] = dtSrc.Rows[i][17].ToString();
            dr["問卷來源"] = dtSrc.Rows[i][18].ToString();

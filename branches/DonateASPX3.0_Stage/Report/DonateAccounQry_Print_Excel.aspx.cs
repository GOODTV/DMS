...
public partial class Report_DonateAccounReport_Print_Excel : BasePage
{
    ...
    private void Print_Excel(string strSql)
    {
        ...
        else
        {
            ...
            //GetTableFooter();
            Util.OutputTxt(GridList.Text, "1", "accoun");
        }
    }
    //---------------------------------------------------------------------------
    private string GetTitle(DataRow dr)
    {
        ...
        string strTitle = "會計科目報表<br/>" + "<span  style=' font-size: 9pt; font-family: 新細明體'>統計日期：" + Session["Date"].ToString()+"</span>";
    ...
    private string GetTable1(DataRow dr, string strSql)
    {
        ...
        foreach (DataRow dw in dtRet.Rows)
        {
            int Col = 0;
            row = new HtmlTableRow();
            foreach (object objItem in dw.ItemArray)
            {
                cell = new HtmlTableCell();
                //cell.Style.Add("border-left", ".5pt solid windowtext");
                cell.Style.Add("border-top", ".5pt solid windowtext");
